...
- `Chat.tsx` 已抽出通知状态、Socket 编排、消息状态、会话列表、详情侧栏、用户搜索和创建会话表单 hook，并已为通知、Socket、会话、消息、详情、用户搜索和创建会话 hook 补上单元测试；创建群/频道弹窗已补组件交互测试，后续风险主要在其他组件交互测试缺口和工程边界清理。
...
3. 在已建立的 Vitest + Testing Library 基线上，继续为消息列表、编辑器、会话列表、详情面板等组件交互补测试。
