...
当前 `Chat.tsx` 已承载较多状态和事件处理，目标拆分如下；其中 `useNotifications`、`useChatSocket` 和 `useMessages` 已先落地：
...
- `Chat.tsx` 已抽出通知状态、Socket 编排和消息状态 hook；会话列表和详情侧栏仍集中在页面组件里，继续加功能会难以维护。
...
3. 继续将 `Chat.tsx` 拆为会话列表和详情侧栏等 hooks，并为已抽出的通知、Socket 与消息 hook 补组件级测试。
