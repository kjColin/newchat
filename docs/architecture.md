...
当前 `Chat.tsx` 已承载较多状态和事件处理，目标拆分如下；其中 `useNotifications`、`useChatSocket`、`useMessages`、`useConversations`、`useConversationDetails`、`useUserSearch` 和 `useCreateConversation` 已先落地：
...
- `Chat.tsx` 已抽出通知状态、Socket 编排、消息状态、会话列表、详情侧栏、用户搜索和创建会话表单 hook；后续风险主要在组件级测试缺口和工程边界清理。
...
3. 为已抽出的通知、Socket、消息、会话、详情、用户搜索和创建会话 hook 补组件级测试。
