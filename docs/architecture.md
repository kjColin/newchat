...
当前 `Chat.tsx` 已承载较多状态和事件处理，目标拆分如下；其中 `useNotifications`、`useChatSocket`、`useMessages`、`useConversations`、`useConversationDetails` 和 `useUserSearch` 已先落地：
...
- `Chat.tsx` 已抽出通知状态、Socket 编排、消息状态、会话列表、详情侧栏和用户搜索 hook；创建会话表单仍集中在页面组件里，继续加功能会难以维护。
...
3. 继续将 `Chat.tsx` 拆为创建群/频道表单等 hooks，并为已抽出的通知、Socket、消息、会话、详情与用户搜索 hook 补组件级测试。
